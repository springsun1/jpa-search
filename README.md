...
### 高级搜索的实现原理
https://docs.efactory.vip:1443/ejpa/adsearchYL.html
...
// RIGHT_LIKE(11, "右模糊查询")
    fieldP = cb.like(root.get(key), val + "%");
以下为测试图片： 全模糊查询： image 左模糊查询： image 右模糊查询： image
~~~
# 3.等于查询、不等于查询
~~~
= 查询
...
终端打印的SQL条件：where student0_.age>=34
测试图片： 小于条件： image 小于等于条件： image 大于条件： image 大于等于条件： image
~~~
# 多条件搜索案例：
# 多条件不包含括号组
...
# 前端使用实例
可以参见系统中已有的一些模块,例如操作日志模块 src/views/monitor/log/index.vue中67行定义了高级搜索支持的字段:
